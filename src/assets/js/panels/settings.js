...
class Settings {
    ...
    async initMods() {
        let modsDatabase = (await this.database.get('1234', 'mods'))?.value;
        let mods = modsDatabase ? modsDatabase : { soop: 'sodium', rm: 'false', lm: 'false' };

        let sodium = document.getElementById("mods-sodium");
        let opti = document.getElementById("mods-optifine");
        let rm = document.getElementById("mods-rm");
        let lm = document.getElementById("mods-lm");
        ...
        sodium.addEventListener("change", () => {
            if(sodium.checked) {
                opti.checked = false;
                mods.soop = 'sodium';
                this.database.update({ uuid: "1234", soop: mods.soop, rm: mods.rm, lm: mods.rm }, 'mods')
            }
        })
        
        opti.addEventListener("change", () => {
            if(opti.checked) {
                sodium.checked = false;
                mods.soop = 'opti';
                this.database.update({ uuid: "1234", soop: mods.soop, rm: mods.rm, lm: mods.rm }, 'mods')
    ...
    async initSettingsDefault() {
        ...
        if (!(await this.database.getAll('mods')).length) {
            this.database.add({ uuid: "1234", soop: 'sodium', rm: 'false', lm: 'false' }, 'mods')
        }
